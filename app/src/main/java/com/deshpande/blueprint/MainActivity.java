...
import com.deshpande.blueprint.adapter.TaskAdapter;
...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, NavigationView.OnNavigationItemSelectedListener {
    ...
    private static final int LOADER_TASK_INBOX = 1;
    private static final int LOADER_TASK_COMPLETED = 2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportLoaderManager().initLoader(LOADER_TASK_INBOX, null, this);
    }
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        } else if (id == LOADER_TASK_INBOX) {
            return new CursorLoader(this, TaskContract.TaskEntry.CONTENT_URI, TaskQuery.PROJECTION,
                    TaskContract.TaskEntry.COLUMN_COMPLETED + "=0",
                    null, null);
        } else if (id == LOADER_TASK_COMPLETED) {
            return new CursorLoader(this, TaskContract.TaskEntry.CONTENT_URI, TaskQuery.PROJECTION,
                    TaskContract.TaskEntry.COLUMN_COMPLETED + "=1",
                    null, null);
        }
        return null;
    }
        ...
        if (loader.getId() == LOADER_PROFILE) {
            ...
            if (photoUri != null) {
                try {
                    ...
                    RoundedBitmapDrawable roundBitmap = RoundedBitmapDrawableFactory.create(getResources(), is);
                    roundBitmap.setCircular(true);
                    userthumbImageView.setImageDrawable(roundBitmap);
                } catch (FileNotFoundException e) {
                    e.printStackTrace();
                }
            }
        } else if (loader.getId() == LOADER_TASK_INBOX) {
            adapter.changeCursor(data);
            recyclerView.setAdapter(adapter);
        } else if (loader.getId() == LOADER_TASK_COMPLETED) {
            adapter.changeCursor(data);
            recyclerView.setAdapter(adapter);
        }
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_completed:
                getSupportLoaderManager().initLoader(LOADER_TASK_COMPLETED, null, this);
                drawerLayout.closeDrawers();
                return true;
        }
        return false;
